# -*- coding: utf-8 -*-
"""
Created on Sun Feb 23 22:11:00 2020
...
gtrends_fm_data=get_daily_trend(pytrend_object, 'farmers market', '2014-01-01', (datetime.now()-timedelta(days=1)).strftime("%Y-%m-%d"), cat=0, \
                    geo='US-CA-803', gprop='', delta=269, overlap=100, sleep=0, \
                    tz=0, verbose=False)

gtrends_fire_data=get_daily_trend(pytrend_object, 'Los Angeles Wildfire', '2014-01-01', (datetime.now()-timedelta(days=1)).strftime("%Y-%m-%d"), cat=0, \
                    geo='US-CA-803', gprop='', delta=269, overlap=100, sleep=0, \
                    tz=0, verbose=False)
...
#now I will calculate movnig average of dataframe values

fm_list=list(joined_gtrends['farmers market'])
lawf_list=list(joined_gtrends['Los Angeles Wildfire'])
...
joined_gtrends['Fire Index']=np.array(previous_fire)
joined_gtrends['FM Index']=np.array(previous_fm)
joined_gtrends=joined_gtrends.rename(columns={"index":'date'})
joined_gtrends=joined_gtrends[['date','Fire Index','FM Index']]
joined_gtrends=joined_gtrends.fillna(joined_gtrends.mean())
joined_gtrends['Fire Index']=joined_gtrends['Fire Index'].apply(lambda x: np.log(x+1)).apply(lambda x: 0 if x<2.5 else 1)

joined_gtrends.to_pickle('gTrends_scraped.pkl')