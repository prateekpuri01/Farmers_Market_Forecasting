...
current_gtrends_info=pd.read_pickle('gTrends_scraped.pkl')

recent_Trends_dates=list(current_gtrends_info['date'])
...
app.layout = html.Div(children=[
    ...
    html.Div([
            ...
            html.Div(html.P(['Real time sales forecasting for Santa Monica Farmers Markets through social media, public city, and geographical data set merging. \
                           Select your market type and product line below to get weekly forecast estimates.', html.Br(), html.Br(), 'Below each forecast you can see \
                           historical information on previous market sales as well as Seed predictions. Curious about what is going on under the hood? Scroll to \
                           the bottom of the dashboard to see the dominant effects that are driving each prediction. Also feel free to check out predicitons for \
                           previous market dates to see what effects are driving your market.']),style={'marginBottom':20}),
...
def output_prediction(forecast_date,selected_market,selected_vendor):
    ...
    if forecast_date in recent_Trends_dates:
        
        trends_df=current_gtrends_info.set_index('date')
        fire_index=trends_df.loc[forecast_date]['Fire Index']
        fm_index=trends_df.loc[forecast_date]['FM Index']
